...
$(".product-image-carousel .image").zoom();

$(document).ready(function () {
  $(".new-sticky-header").sticky({ topSpacing: 0 });

  //categories bar
  $(".btn_catalog, .header-menu-icon, .btn-navicon").click(function () {
    $(".product-categories-wrap").css("display", "block");
  });

  $(".close-button").click(function () {
    $(".product-categories-wrap").css("display", "none");
  });

  $(document).click(function (event) {
    //if you click on anything except the modal itself or the "open modal" link, close the modal
    if ($(event.target).hasClass("reveal-overlay")) {
      $("body").find(".product-categories-wrap").css("display", "none");
    }
    if (
      !$(event.target).closest(
        ".product-categories-wrap,.btn_catalog, .header-menu-icon, .btn-navicon, .all_categories_btn"
      ).length
    ) {
      $("body").find(".product-categories-wrap").css("display", "none");
    }
  ...
  $(".product-image-carousel .slick-slider").slick({
    dots: false,
    nav: false,
    infinite: true,
    slidesToShow: 1,
  ...
  function scrollY(last_known_scroll_position) {
    if (last_known_scroll_position > $(".header_bottom").height()) {
  ...
  //accordion
  $(".facet-box__header").click(function () {
    $(this)
      .closest(".facet-box")
      .find(".facet-box__body")
      .toggleClass("active")
      .toggle();
  });

function showProducts(minPrice, maxPrice) {
    $("[data-price]").hide().filter(function() {
...
$(".price-field-max").val(max);
  ...
  $("#slider-range").slider({
    ...
    slide: function (event, ui) {
      ...
      $(".price-field-min").val(minVal);
      $(".price-field-max").val(maxVal);
